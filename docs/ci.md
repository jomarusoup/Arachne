...
flowchart TB
    ...
    R --> R1["dnf + EPEL<br/>ShellCheck bats git jq"]
...
flowchart TB
    ...
    C --> D["dnf install<br/>ShellCheck bats git jq"]
    D --> E["ShellCheck"]
    E --> F["Bats 전체"]
...
dnf install -y ShellCheck bats git jq
shellcheck -S warning ./*.sh hooks/*.sh tests/*.sh
bats tests/*.bats
bash tests/validate_settings.sh
bash tests/check_index.sh
...
Windows job은 PowerShell 설치기와 Git Bash 스모크를 분리한다. 현재 반복 실패했던 지점은
`tests/install_windows.ps1`가 `install.ps1` 호출 인자를 함수에 넘길 때 배열 파라미터를 명시하지 않아
`-Install`이 `Target` positional 값처럼 해석된 문제였다.
...
flowchart TB
    ...
    P --> P1{"오류 유형"}
    P1 -->|ValidateSet Target| P2["PowerShell 인자 전달<br/>-Arguments 배열 명시 여부 확인"]
...
GNU 도구 경로를 `GITHUB_PATH`에 추가한다.

```mermaid
flowchart TB
    A["macos-latest"] --> B["brew install<br/>shellcheck bats-core jq coreutils"]
    B --> C["coreutils gnubin<br/>GITHUB_PATH 추가"]
    C --> D["ShellCheck"]
    D --> E["Bats 전체"]
    E --> F["settings.template.json"]
    F --> G["문서 인덱스"]
