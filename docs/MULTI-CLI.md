...
flowchart TB
    ...
    GIT -.->|"git-bus: 업스트림 새 커밋 감지<br/>(hooks/gemini-check.sh)"| CLAUDE
...
또 다른 채널은 **git-bus**다. `hooks/gemini-check.sh`는 업스트림 브랜치 HEAD와 기준점을 비교해
다음 프롬프트 때 새 커밋을 알린다. 작성자나 사용 CLI는 판별하지 않으며, 업스트림이 있는 경우
다른 로컬 터미널의 미푸시 커밋은 감지 대상이 아니다.
