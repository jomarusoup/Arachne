...
- [ ] `docs/template/feedback.md`에 필수 필드와 민감정보 점검 항목을 정의한다.
- [ ] 신규 프로젝트 스캐폴딩에 `docs/feedback/`과 피드백 템플릿을 포함한다.
- [ ] `arachne feedback new`가 날짜 기반 파일명으로 로컬 초안을 생성한다.
- [ ] `arachne feedback list`가 제출 상태별 피드백 목록을 출력한다.
- [ ] `arachne feedback submit`이 제출 내용을 먼저 출력하고 명시적 확인을 요구한다.
- [ ] `submit`이 `gh` 설치·인증·저장소 접근 가능 여부를 검증한 뒤 GitHub Issue를 생성한다.
- [ ] 제출 성공 시 Issue URL과 제출 시각을 로컬 피드백 문서에 기록한다.
- [ ] 제출 실패 시 원본 문서를 변경하지 않고 원인을 사용자에게 출력한다.
- [ ] 토큰 형태와 절대 경로 등 명백한 민감정보 후보가 있으면 기본 제출을 중단한다.
- [ ] 동일 피드백의 중복 제출을 방지하거나 명확히 경고한다.
- [ ] Bash와 Windows PowerShell 경로에 대한 단위·통합 테스트를 추가한다.
- [ ] README와 사용 가이드에 issue, task, feedback의 역할과 전달 흐름을 문서화한다.
