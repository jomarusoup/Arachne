---
Title: "[idea] everything-claude-code Python·Web 선별 도입 검토"
creation: 2026-06-09
...
tags:
 ...
 - "web"
aliases:
 - "ecc-python-web-gap-analysis"
---
MOC:: [[Arachne]]
FROM:: [[2026-06-09-python-web-harness-assessment]]

# [idea] everything-claude-code Python·Web 선별 도입 검토

- **비교 원본**: `/home/Harness/everything-claude-code`
- **원본 revision**: `e3a25791`
- **원본 버전**: `2.0.0-rc.1`
- **라이선스**: MIT
...
| 표면 | Arachne | ECC 로컬 clone | 판단 |
| --- | ---: | ---: | --- |
| agents | 7 | 63 | ECC 전체 도입 시 선택 비용 급증 |
| commands | 16 | 79 | 중복·별칭·오케스트레이션 충돌 위험 |
| rules 파일 | 49 | 115 | 언어 깊이는 ECC가 우세 |
| skills | 28 | 404 | 전체 설치는 Arachne 철학과 맞지 않음 |
| hooks | 6 | 4개 최상위 + 런타임 모듈 | 파일 수보다 설치·프로필 구조가 중요 |

ECC의 강점은 “많은 파일”보다 선택적 설치, provenance, context budget, quality gate, 프레임워크별
...
```mermaid
flowchart TD
    A["ECC 자산"] --> B{"Python/Web 우선순위와 직접 관련?"}
    B -->|no| X["도입하지 않음"]
...
FastAPI 관련 ECC 자산은 Arachne에도 대부분 존재한다. 새 파일을 복사하기보다 아래 누락만 기존
...
| skill placement/provenance policy | 높음 | 외부 도입·학습 스킬의 출처와 업데이트 추적 |
...
Arachne의 Claude 중심 3-레인과 ECC의 multi-* 명령을 동시에 설치하면 책임 경계가 중복된다.

### 4. continuous learning 자동 승격

학습된 규칙이 검토 없이 전역 규약으로 올라가면 잘못된 패턴이 고착된다. provenance, confidence,
승인 게이트가 먼저 필요하다.

### 5. plugin/installer를 중첩 설치

ECC README도 plugin과 full installer를 함께 쓰면 duplicate hooks/skills가 생긴다고 경고한다.
Arachne와 ECC를 동시에 전역 설치하기보다 참고 원본으로 사용해야 한다.
...
1. selective profile과 provenance 정책부터 만든다.
...
ECC는 MIT이므로 재사용할 수 있지만, 실질적인 문구나 코드 샘플을 가져오면 원 저작권 고지와 출처를
유지해야 한다. Arachne 문체와 구조에 맞게 재작성하더라도 아이디어 문서 또는 해당 skill frontmatter에
원본 경로와 revision을 기록하는 편이 안전하다.
