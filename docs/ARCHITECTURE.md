...
- **병렬성** — Claude 내장 서브에이전트(`Agent` 도구, `agents/` 8종)로 확보한다.
  독립 작업은 동시 실행, 파일을 수정하는 병렬 작업은 worktree로 격리한다.
- **게이트** — 커밋 전 `/verify`(정적+동작) + 프로젝트 CI(`.arachne/verify.sh`·GitHub Actions).
- **맹점 보정** — 구현과 검증이 같은 모델이므로 `code-reviewer` 리뷰와 `/verify`를 한 단계 더
  신중하게 적용한다(`rules/common/workflow.md`).
...
```
Arachne/
├── AGENTS.md                    # 공통 규약 SSOT (다중 CLI 어댑터로 배포)
├── archive/                     # 제거된 현역 외 자산 (multi-cli — ADR-0004)
...
`조사 → 설계 → TDD → 리뷰 → 커밋`. 전 단계를 Claude가 수행하되, 단계별로 전담
서브에이전트(planner·tdd·code-reviewer·debugger)를 활용한다.
