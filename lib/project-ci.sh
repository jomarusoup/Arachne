...
InstallProjectDesignDocs() {
    ...
    ProfileHasDesignDocs "$profile" || return 0
    if [ ! -f "$design_tmpl" ]; then
        ArachneLog "ERROR" "디자인 문서 템플릿이 없습니다: $design_tmpl"
        return 1
    fi
    for managed_path in "$design_dir" "$design_file" "$decisions_dir" "$legacy_file"; do
        if [ -L "$managed_path" ]; then
            ArachneLog "ERROR" "디자인 문서 경로가 심볼릭 링크입니다: $managed_path"
            return 1
        fi
    done
...
InitProjectCi() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            --profile)
                shift
                [ $# -gt 0 ] || {
                    ArachneLog "ERROR" "--profile 값이 필요합니다"
                    return 1
                }
                profile="$1"
                ;;
            -*) ArachneLog "ERROR" "알 수 없는 옵션: $1"; return 1 ;;
            *) positional+=("$1") ;;
        esac
        shift
    done
    ...
    if [ "${#positional[@]}" -gt 1 ]; then
        ArachneLog "ERROR" "프로젝트 디렉터리는 하나만 지정할 수 있습니다"
        return 1
    fi
    ValidateProjectProfile "$profile" || return 1
    ...
    ArachneSection "프로젝트 CI 초기화 시작 (profile=$profile)"
    if [ ! -d "$project_dir" ]; then
        ArachneLog "ERROR" "프로젝트 디렉터리가 없습니다: $project_dir"
        return 1
    fi
    if [ ! -f "$verify_tmpl" ] || [ ! -f "$commands_tmpl" ] \
        || [ ! -f "$workflow_tmpl" ]; then
        ArachneLog "ERROR" "프로젝트 CI 템플릿이 없습니다: $REPO_DIR/templates/project"
        return 1
    fi
    ...
    for managed_path in \
        ...
        if [ -L "$managed_path" ]; then
            ArachneLog "ERROR" "프로젝트 CI 관리 경로가 심볼릭 링크입니다: $managed_path"
            return 1
        fi
    done
    ...
    InstallProjectDesignDocs "$project_abs" "$profile" || return 1
...
CheckProject() {
    ...
    if [ ! -f "$verify_script" ]; then
        ArachneLog "ERROR" "프로젝트 CI가 초기화되지 않았습니다: arachne init-ci \"$project_abs\""
        return 1
    fi

    if bash "$verify_script"; then
        ArachneSection "프로젝트 검증 완료"
        return 0
    else
        ArachneSection "프로젝트 검증 실패"
        return 1
    fi
}
...
NewProject() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            --profile)
                ...
                profile="$1"
                ;;
            -*)       ArachneLog "ERROR" "알 수 없는 옵션: $1"; exit 1 ;;
            *)        positional+=("$1") ;;
        esac
        shift
    done
    ...
    # 입력 검증
    #---------------------------------------------------------------------------
    ArachneSection "신규 프로젝트 생성 시작"
    if [ -z "$proj" ]; then
        ArachneLog "ERROR" "프로젝트명이 필요합니다: arachne new <project> [parent-dir] [--no-git]"
        exit 1
    fi
    case "$proj" in
        */*|.*) ArachneLog "ERROR" "프로젝트명에 '/'·선행 '.' 사용 불가: $proj"; exit 1 ;;
    esac
    ...
    if [ -e "$dest" ]; then
        ArachneLog "ERROR" "이미 존재합니다: $dest"
        exit 1
    fi
    ...
    #---------------------------------------------------------------------------
    InitProjectCi "$dest" --profile "$profile"
