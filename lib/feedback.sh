...
#               단독 실행하지 않는다 (REPO_DIR·ArachneLog·ArachneSection 은
#               install.sh 가 정의).
# DATA        : 2026-07-02
...
CreateFeedbackDraft() {
    ...
    if [ ! -f "$feedback_tmpl" ]; then
        ArachneLog "ERROR" "피드백 템플릿이 없습니다: $feedback_tmpl"
        return 1
    fi
    ArachneSection "feedback 초안 생성 시작"
    project_abs=$(pwd)
    ...
    ArachneSection "feedback 초안 생성 완료"
    echo "$feedback_file"
}

#===============================================================================
# FUNCTION    : ListFeedbackDrafts
...
SubmitFeedback() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            -*) ArachneLog "ERROR" "알 수 없는 feedback submit 옵션: $1"; return 1 ;;
            *)  feedback_file="$1" ;;
        esac
        shift
    done
    ArachneSection "feedback 제출 시작"
    if [ -z "$feedback_file" ] || [ ! -f "$feedback_file" ]; then
        ArachneLog "ERROR" "제출할 feedback 파일이 필요합니다"
        return 1
    fi
    if grep -Eq '^status: "submitted"|^- \*\*상태\*\*: submitted' "$feedback_file"; then
        ArachneLog "ERROR" "이미 제출된 feedback 입니다: $feedback_file"
        return 1
    fi
    [ "$allow_sensitive" -eq 1 ] || AssertFeedbackSafe "$feedback_file" || return 1

    command -v gh >/dev/null 2>&1 || {
        ArachneLog "ERROR" "gh CLI가 필요합니다"
        return 1
    }
    gh auth status >/dev/null 2>&1 || {
        ArachneLog "ERROR" "gh 인증 상태를 확인할 수 없습니다"
        return 1
    }
    gh repo view >/dev/null 2>&1 || {
        ArachneLog "ERROR" "현재 디렉터리에서 GitHub 저장소를 확인할 수 없습니다"
        return 1
    }

    ArachneSection "feedback 제출 미리보기: $feedback_file"
    sed -n '1,220p' "$feedback_file"
    if [ "${ARACHNE_FEEDBACK_YES:-0}" != "1" ]; then
        printf 'Submit to GitHub Issue? Type YES: '
        read -r answer
        if [ "$answer" != "YES" ]; then
            ArachneLog "SKIP" "feedback 제출 취소"
            return 1
        fi
    fi
    ...
    ArachneSection "feedback 제출 완료: $issue_url"
}
...
FeedbackCommand() {
    ...
    case "$subcommand" in
        new)    shift; CreateFeedbackDraft "$*" ;;
        list)   ListFeedbackDrafts ;;
        submit) shift; SubmitFeedback "$@" ;;
        *)
